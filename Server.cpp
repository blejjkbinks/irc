...
Server::Server(int port, std::string password)
    : _listen_port(port), _password(password), _server_name("ircserv"),
...
Server::Server(void)
    : _listen_port(0), _password(""), _server_name("ircserv"), _clients_n(0),
      _channels_n(0) {}
  ...
  if (pfds[i].revents & (POLLIN | POLLERR | POLLHUP)) {
    ...
    if (n > 0) {
      ...
      while ((pos = b.find('\n')) != std::string::npos) {
        ...
        _clients[i - 2].process_line(line, *this, i - 1);
