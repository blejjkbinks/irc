...
void Client::_privmsg(std::string line, Server &server) {
  ...
  std::string full_message = ":" + _nick + "!" + _user + "@localhost PRIVMSG " +
                             target + " :" + message + "\r\n";
...
void Client::_ping(std::string line, Server &server) {
  ...
  std::string reply =
      "PONG " + server.getServerPrefix().substr(1) + " " + token + "\r\n";
  ft_send(reply);
}
...
void Client::process_line(std::string line, Server &server, int index) {
  ...
  if (!_is_registered) {
    std::string reply = "451 " + (command.empty() ? "*" : command) +
...
void Client::_join(std::string line, Server &server) {
  ...
  if (!channel) {
    ...
    std::string join_msg =
        ":" + _nick + "!" + _user + "@localhost JOIN " + channel_name + "\r\n";
...
void Client::_kick(std::string line, Server &server) {
  ...
  std::string kick_msg = ":" + _nick + "!" + _user + "@localhost KICK " +
...
void Client::_invite(std::string line, Server &server) {
  ...
  target->ft_send(":" + _nick + "!" + _user + "@localhost INVITE " +
...
void Client::_topic(std::string line, Server &server) {
  ...
  if (second_space == std::string::npos) {
    ...
    std::string topic_msg = ":" + _nick + "!" + _user + "@localhost TOPIC " +
                            channel_name + " :" + topic + "\r\n";
...
void Client::_mode(std::string line, Server &server) {
  ...
  // Broadcast mode change (simplified, ideally construct the actual change
  // string)
  std::string mode_msg = ":" + _nick + "!" + _user + "@localhost MODE " +
                         channel_name + " " + modes + " " + args + "\r\n";
...
void Client::_part(std::string line, Server &server) {
  ...
  while ((pos = channels_str.find(',')) != std::string::npos) {
    ...
    if (c) {
      std::string part_msg = ":" + _nick + "!" + _user + "@localhost PART " +
                             channel_name + " :" + reason + "\r\n";
