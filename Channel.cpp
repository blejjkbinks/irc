...
Channel::Channel(void)
    : _clients_n(0), _name(""), _topic(""), _key(""), _limit(0), _mode_i(false),
...
void Channel::removeOperator(std::string nick) {
  std::vector<std::string>::iterator it =
      std::find(_operators.begin(), _operators.end(), nick);
  if (it != _operators.end())
    _operators.erase(it);
}

bool Channel::isOperator(std::string nick) {
  return std::find(_operators.begin(), _operators.end(), nick) !=
         _operators.end();
}
